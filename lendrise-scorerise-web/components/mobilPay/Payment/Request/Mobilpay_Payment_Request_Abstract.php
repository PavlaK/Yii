...
abstract class Mobilpay_Payment_Request_Abstract
{
    ...
    static protected function _factoryFromXml(DOMDocument $xmlDoc)
    {
        ...
        if ($elems->length != 1) {
            throw new Exception('factoryFromXml order element not found',
                Mobilpay_Payment_Request_Abstract::ERROR_FACTORY_BY_XML_ORDER_ELEM_NOT_FOUND);
        }
        $orderElem = $elems->item(0);

        $attr = $orderElem->attributes->getNamedItem('type');
        if ($attr == null || strlen($attr->nodeValue) == 0) {
            throw new Exception('factoryFromXml invalid payment request type=' . $attr->nodeValue,
    ...
    protected function _parseFromXml(DOMNode $elem)
    {
        ...
        if ($xmlAttr == null || strlen((string)$xmlAttr->nodeValue) == 0) {
            throw new Exception('Mobilpay_Payment_Request_Sms::_parseFromXml failed: empty order id',
        ...
        if ($elems->length != 1) {
            throw new Exception('Mobilpay_Payment_Request_Sms::loadFromXml failed: signature is missing',
