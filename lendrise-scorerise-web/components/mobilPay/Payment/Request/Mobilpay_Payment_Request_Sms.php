...
namespace app\components\mobilPay\Payment\Request;
...
class Mobilpay_Payment_Request_Sms extends Mobilpay_Payment_Request_Abstract
{
    ...
    protected function _loadFromQueryString($queryString)
    {
        ...
        if (!isset($reqParams['signature'])) {
            throw new Exception('Mobilpay_Payment_Request_Sms::loadFromQueryString failed: signature is missing',
                self::ERROR_LOAD_FROM_XML_SIGNATURE_ELEM_MISSING);
        }
        $this->signature = $reqParams['signature'];
        if (!isset($reqParams['service'])) {
            throw new Exception('Mobilpay_Payment_Request_Sms::loadFromQueryString failed: service is missing',
                self::ERROR_LOAD_FROM_XML_SERVICE_ELEM_MISSING);
        }
        $this->service = $reqParams['service'];
        if (!isset($reqParams['tran_id'])) {
            throw new Exception('Mobilpay_Payment_Request_Sms::loadFromQueryString failed: empty order id',
    ...
    protected function _prepare()
    {
        if (is_null($this->signature) || is_null($this->service) || is_null($this->orderId)) {
            throw new Exception('One or more mandatory properties are invalid!',
