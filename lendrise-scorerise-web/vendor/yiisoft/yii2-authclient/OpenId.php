...
class OpenId extends BaseClient implements ClientInterface
{
    ...
    protected function extractHtmlTagValue(
        ...
        preg_match_all("#<{$tag}[^>]*$matchAttributeName=['\"].*?$matchAttributeValue.*?['\"][^>]*$valueAttributeName=['\"](.+?)['\"][^>]*/?>#i",
            $content, $matches1);
        preg_match_all("#<{$tag}[^>]*$valueAttributeName=['\"](.+?)['\"][^>]*$matchAttributeName=['\"].*?$matchAttributeValue.*?['\"][^>]*/?>#i",
            $content, $matches2);
                ...
                if (isset($headers['content-type'])
                    ...
                    foreach ($m[1] as $content) {
                        ...
                        if (preg_match('#<Type>\s*' . $ns . '(server|signon)\s*</Type>#s', $content, $type)) {
                            ...
                            $result['sreg'] = strpos($content,
                                    '<Type>http://openid.net/sreg/1.0</Type>') || strpos($content,
